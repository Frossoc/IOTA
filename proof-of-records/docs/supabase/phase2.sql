-- Phase 2: API keys, usage counters, audit logs
-- Additive migration (keeps Phase 1 schema untouched)
...
create index if not exists usage_counters_por_api_key_idx
...
create index if not exists audit_logs_por_client_time_idx
  on public.audit_logs_por(client_id, created_at desc);
